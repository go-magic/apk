...
func FromReaderWithConfig(r io.ReaderAt, size int64, config *ResTableConfig) (*APK, error) {
	...
	if err = xml.Unmarshal([]byte(xmlFile.String()), &apk.manifest); err != nil {
		if apk.manifest.Package == "" || apk.manifest.Application.Label == nil || *apk.manifest.Application.Label == "" {
			return nil, err
		}
	}
...
func(a *APK) ReadXmlFile(xml string) (*XMLFile, error) {
	return NewXMLFile(bytes.NewReader([]byte(xml)), a.table, nil)
}
